...
@app.route('/inventory/<user_id>/<company_id>', methods=['GET'])
def inventory(user_id, company_id):
    global dataSet, real_trend, parameters, minmax, scaled_parameters, initial_money, agent

    account_response = requests.get(f'http://localhost:8080/api/bankAccount/{user_id}')
    inventory_response = requests.get(
        f'http://localhost:8080/api/accountInventory/{account_response.json()["account_id"]}/{company_id}')

    data_set = pd.read_csv('../dataset/' + inventory_response.json()['company']['ticker'] + '.csv')
    real_trend = data_set['Close'].tolist()
    parameters = [data_set['Close'].tolist(), data_set['Volume'].tolist()]
    ...
    return jsonify(inventory_response.json())
...
@app.route('/trade_ai/{company}', methods=['POST'])
def trade_ai():
    # Load the company's data
    dataSet = pd.read_csv(f'../dataset/{request.view_args["company"]}.csv')
    close = dataSet['Close'].tolist()
    volume = dataSet['Volume'].tolist()

    # Get the initial state based on the last known data
    last_state = agent.get_state(
        len(close) - 1,
        agent._inventory,
        agent._scaled_capital,
        timeseries=[close, volume]
    )

    # Start an infinite loop of trading without data
    while True:
        # Predict an action based on the current state
        action = agent.act(last_state)

        # Execute the predicted action
        if action == 1 and agent._scaled_capital >= close[-1]:
            # Buy
            agent._inventory.append(close[-1])
            agent._scaled_capital -= close[-1]
        elif action == 2 and len(agent._inventory):
            # Sell
            bought_price = agent._inventory.pop(0)
            agent._scaled_capital += close[-1]
        ...
        # Print the current action and balance for debugging
        print(f"Action: {'Buy' if action == 1 else 'Sell' if action == 2 else 'Hold'}, Balance: {agent._capital}")

        # Sleep for a while before the next iteration
        time.sleep(1)


with open('../AI/model.pkl', 'rb') as fopen:
...
#     requests.get('http://10.139.2.154:8005/balance').json()
#     data = json.dumps([close[0], volume[0]])
#     requests.get('http://10.139.2.154:8005/trade?data=' + data).json()
#
#     for i in range(200):
#         data = json.dumps([close[i], volume[i]])
#         requested = requests.get('http://10.139.2.154:8005/trade?data=' + data).json()
#         print(requested, date[i])
#     return jsonify(True)


# df = pd.read_csv('../AI/TWTR.csv')
